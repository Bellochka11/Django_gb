...
# (venv) PS C:\Users\BOSS\Desktop\Django_code\myproject> python .\manage.py fill_db 10 создаем 10 авторов у одного автора 10 постов
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        text = LOREM.split()
        count = kwargs.get('count') #извлекаем из командной строки количесвто которое передли сколько пользователей
        for i in range(1, count + 1):
            author = Author(name=f'Author_{i}', email=f'mail{i}@mail.ru') #создаем авторов
            author.save()
            for j in range(1, count + 1):
                post = Post(
                    title=f'Title-{j}',
                    content=" ".join(choices(text, k=64)),#случайным образом возьми строку из 64 слов и сформируй текст раздели пробелом
                    author=author
                )
                post.save()